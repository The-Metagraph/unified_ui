...
make frontend-event-route-key-source-requirements-validate
make frontend-event-route-key-source-keys-validate
make frontend-event-route-key-source-key-parity-validate
make frontend-event-route-key-source-map-parity-validate
make frontend-event-route-key-source-triad-parity-validate
```
...
Validate frontend widget event payload-key parity against `WebUi.Events.EventCatalog`:

```bash
./scripts/validate_frontend_event_payload_contract.sh
```

Validate frontend widget event route-family and route-key parity against canonical route conventions:

```bash
./scripts/validate_frontend_event_route_contract.sh
```

Validate frontend widget event `route_family` continuity parity against canonical event route mappings:
...
Validate frontend widget event `route_keys` ordered continuity parity against canonical route-key ordering requirements:
...
Validate frontend widget event `route_keys` allowlist parity against canonical route-key requirements:

```bash
./scripts/validate_frontend_event_route_key_allowlist_contract.sh
```

Validate frontend widget event `route_keys` payload-shape parity against canonical route-key requirements:

```bash
./scripts/validate_frontend_event_route_key_shape_contract.sh
```

Validate frontend widget event route-key value-shape parity against canonical route-key requirements:
...
Validate frontend widget event route-key source-requirements parity against canonical route-key source conventions:

```bash
./scripts/validate_frontend_event_route_key_source_requirements_contract.sh
```

Validate frontend widget event route-key source-key parity against canonical route-key source continuity conventions:

```bash
./scripts/validate_frontend_event_route_key_source_keys_contract.sh
```

Validate frontend widget event route-key source-key parity against emitted `route_keys` continuity:

```bash
./scripts/validate_frontend_event_route_key_source_key_parity_contract.sh
```

Validate frontend widget event route-key source-map parity against emitted `route_keys` continuity:

```bash
./scripts/validate_frontend_event_route_key_source_map_parity_contract.sh
```

Validate frontend widget event route-key source triad parity across `route_keys`, `route_key_source_keys`, and `route_key_sources` continuity:
...
- `pre-commit`: specs governance + RFC governance + frontend wiring checks + frontend transport contract parity + frontend CloudEvent contract parity + frontend runtime-context contract parity + frontend event catalog contract parity + frontend event payload contract parity + frontend event route contract parity + frontend event route-family contract parity + frontend event route-keys contract parity + frontend event route-key ordering parity + frontend event route-key completeness parity + frontend event route-key allowlist parity + frontend event route-key payload-shape parity + frontend event route-key value-shape parity + frontend event route-key value-source parity + frontend event route-key source-requirements parity + frontend event route-key source-key parity + frontend event route-key source-key to route-key parity + frontend event route-key source-map to route-key parity + frontend event route-key source triad parity.
