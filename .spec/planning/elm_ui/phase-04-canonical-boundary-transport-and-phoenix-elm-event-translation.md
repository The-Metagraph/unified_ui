...
- `elm_ui` may use its own native Phoenix-and-Elm interaction model locally, but canonical meaning must be preserved at package boundaries.
...
[x] 4 Phase 4 - Canonical Boundary Transport and Phoenix-Elm Event Translation
  Implement the canonical boundary transport model, split-runtime event translation surfaces, and diagnostics that let `elm_ui` speak ecosystem-wide canonical event meaning.
