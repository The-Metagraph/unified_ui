...
[x] 2 Phase 2 - Foundational Native Widgets and Baseline IUR Rendering
  Implement foundational native widgets, baseline forms and navigation, and the first canonical `UnifiedIUR` rendering path across the split `elm_ui` runtime.

  [x] 2.1 Section - Foundational Native Widget Families
    Implement the core widget families that make direct-use `elm_ui` useful before advanced display systems arrive.
  ...
  [x] 2.3 Section - Baseline Canonical IUR Rendering
    Implement the first canonical renderer path that maps foundational `UnifiedIUR` widgets into native `elm_ui` widgets.
  ...
  [x] 2.4 Section - Foundational Reference Examples
    ...
    [x] 2.4.1 Task - Implement foundational native and canonical examples
      Provide review-friendly examples that demonstrate the baseline `elm_ui` runtime model and the first canonical renderer path.
  ...
  [x] 2.5 Section - Phase 2 Integration Tests
    ...
    [x] 2.5.2 Task - Foundational canonical renderer scenarios
      ...
      [x] 2.5.2.1 Subtask - Verify foundational canonical widgets render through native `elm_ui` widget reuse rather than a separate renderer stack.
