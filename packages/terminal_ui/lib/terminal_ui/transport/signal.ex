defmodule TerminalUi.Transport.Signal do
  @moduledoc """
  Canonical `Jido.Signal` translation for `terminal_ui`.
  """
  ...
  @boundary_crossing_families [:submit, :selection, :navigation, :command]
    ...
    with :ok <- validate_family(family),
      ...
      translation =
        %{
          ...
          backend_mode: Map.get(normalized, :backend_mode, :raw),
  ...
  def from_interaction(interaction, attrs \\ []) do
    ...
    normalized = %{
      ...
      backend_mode: Map.get(attrs, :backend_mode, :raw),
      input_family: Map.get(attrs, :input_family, :key),
  ...
  def from_boundary_signal(%Signal{} = signal) do
    family = extension(signal, :terminal_ui_family)
    target = normalize_map(extension(signal, :terminal_ui_target, %{}))
    ...
    with :ok <- validate_family(family),
      ...
      {:ok,
       %{
         boundary: :boundary,
         family: family,
         intent: extension(signal, :terminal_ui_intent),
         runtime_event: extension(signal, :terminal_ui_runtime_event),
         source_kind: extension(signal, :terminal_ui_source_kind, :canonical),
         backend_mode: extension(signal, :terminal_ui_backend_mode, :raw),
         input_family: extension(signal, :terminal_ui_input_family),
         widget_id: signal.subject,
         runtime_id: extension(signal, :terminal_ui_runtime_id),
         screen: extension(signal, :terminal_ui_screen, "unknown"),
         target: target,
         payload: normalize_map(signal.data || %{}),
         normalized_input: %{},
         local_handling: extension(signal, :terminal_ui_local_handling),
         signal: signal,
  ...
  defp build_signal(translation) do
    Signal.new(
      ...
      extensions: %{
        terminal_ui_family: translation.family,
        terminal_ui_intent: translation.intent,
        terminal_ui_runtime_event: translation.runtime_event,
        terminal_ui_source_kind: translation.source_kind,
        terminal_ui_backend_mode: translation.backend_mode,
        terminal_ui_input_family: translation.input_family,
        terminal_ui_runtime_id: translation.runtime_id,
        terminal_ui_screen: translation.screen,
        terminal_ui_target: translation.target,
        terminal_ui_local_handling: translation.local_handling
      }
    )
  end

  defp cloud_event_type(family, intent) do
    ["terminal_ui", family, intent]
  ...
  defp cloud_event_source(source_kind, screen) do
    "/terminal_ui/#{normalize_segment(source_kind)}/#{normalize_segment(screen)}"
  end
