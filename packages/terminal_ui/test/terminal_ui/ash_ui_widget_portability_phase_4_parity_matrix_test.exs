defmodule TerminalUi.AshUiWidgetPortabilityPhase4ParityMatrixTest do
  ...
  test "parity matrix covers every promoted canonical widget kind" do
    ...
    assert Enum.all?(matrix_kinds, &(&1 in TerminalUi.Renderer.supported_kinds()))
    assert Enum.all?(matrix_kinds, &(&1 in TerminalUi.Widgets.kinds()))
  end

  test "terminal renderer realizes every matrix widget with declared fallback metadata" do
    for entry <- matrix_entries() do
      kind = kind(entry)
      terminal = runtime_entry(entry, "terminal")

      assert terminal["support"] in ["direct", "fallback"]
      assert {:ok, %{kind: ^kind} = widget} = TerminalUi.Renderer.render(canonical_element(kind))

      case terminal["fallback"] do
        nil ->
          assert TerminalUi.Degradation.resolve(widget, backend_mode: :tty) == nil

        fallback ->
          assert TerminalUi.Degradation.resolve(widget, backend_mode: :tty) ==
                   String.to_existing_atom(fallback)
      end
    end
  end

  test "terminal native events match matrix interaction expectations" do
    for entry <- matrix_entries() do
      required_events = required_events(entry, "terminal")
      native_widget = native_widget(kind(entry))

      assert Enum.all?(required_events, &Map.has_key?(native_widget.events, &1)),
             "terminal #{entry["kind"]} missing events #{inspect(required_events)}"
    end
  end

  test "terminal repeated collection validation preserves row-scope diagnostics and payloads" do
    assert {:ok, root} = TerminalUi.Renderer.render(row_scope_collection())
  ...
  defp native_widget(:disclosure),
    do:
      TerminalUi.Widgets.disclosure("native-disclosure", "Details", on_toggle: %{intent: :toggle})

  defp native_widget(:kicker), do: TerminalUi.Widgets.kicker("native-kicker", "Workflow")
  defp native_widget(:avatar), do: TerminalUi.Widgets.avatar("native-avatar", "Pat")
