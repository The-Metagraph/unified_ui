defmodule DesktopUi.Sdl3RenderPlanTest do
  ...
  test "native window sessions distinguish top-level windows from transient layers" do
    ...
    assert Enum.map(registry.sessions, & &1.id) == ["window:details-window", "window:operations-window"]
    ...
    assert :ok =
             Window.validate_transition(registry, "window:operations-window", "window:details-window")
  end
