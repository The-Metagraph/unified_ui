...
echo "Validating frontend widget event route-key allowlist parity..."
...
if ! file_contains_literal "routeFamilyCompatibilityRouteKeys" "$ELM_FILE"; then
  fail "Elm harness missing route-key compatibility allowlist derivation helper"
fi

if ! file_contains_literal "isAllowedRouteKey" "$ELM_FILE"; then
  fail "Elm harness missing route-key allowlist membership helper"
fi

if ! file_contains_literal "unexpectedRouteKeys" "$BRIDGE_FILE"; then
  fail "JS bridge missing unexpected route-key detection helper"
fi

if ! file_contains_literal "unexpected_route_keys" "$BRIDGE_FILE"; then
  fail "JS bridge missing unexpected route-key diagnostics"
fi

if ! file_contains_literal "allowed_route_keys" "$BRIDGE_FILE"; then
  fail "JS bridge missing allowed route-key diagnostics"
fi

if ! file_contains_literal "non-canonical route keys for route family" "$BRIDGE_FILE"; then
  fail "JS bridge missing non-canonical route-key fail-closed reason"
fi
...
if [[ "$failures" -ne 0 ]]; then
  echo
  echo "Frontend widget event route-key allowlist validation failed."
  exit 1
fi

echo "Frontend widget event route-key allowlist validation passed."
