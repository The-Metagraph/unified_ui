defmodule WebUi.PhaseFourIntegrationTest do
  ...
  test "canonical boundary events round-trip through jido signal semantics and server acknowledgements" do
    assert {:ok, runtime_state} =
             WebUi.Runtime.mount_iur_screen(
               WebUi.Examples.canonical_transport_screen(),
    ...
    assert event_message.payload.type == "web_ui.submit.save_workspace"
    ...
    assert {:ok, next_state, ack_message} =
             WebUi.Runtime.handle_frontend_event(runtime_state, event_message)

    assert Enum.at(next_state.event_log, -1).mode == :boundary
    assert next_state.last_boundary_signal.type == "web_ui.submit.save_workspace"
  ...
  test "server-side runtime remains authoritative across translated events and frontend updates" do
    ...
    assert {:ok, frontend_after_ack} =
             WebUi.FrontendRuntime.apply_server_message(frontend_after_dispatch, ack_message)
  ...
  test "bounded elm-side behavior contributes responsiveness without displacing server authority" do
    assert {:ok, runtime_state} =
             WebUi.Runtime.mount_native_screen(
               WebUi.Examples.native_transport_screen(),
  ...
  test "invalid boundary leakage fails with actionable diagnostics" do
    ...
    assert {:error, %ServerRuntime.Error{reason: :invalid_frontend_payload, details: details}} =
             WebUi.Runtime.handle_frontend_event(runtime_state, leaked_message)
    ...
    assert {:error,
            %ServerRuntime.Error{
             ...
             WebUi.Runtime.handle_boundary_envelope(runtime_state, %{
