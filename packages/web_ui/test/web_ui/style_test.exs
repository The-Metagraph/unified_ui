defmodule WebUi.StyleTest do
  ...
  test "style primitives expose portable native styling categories" do
    ...
    assert :theme_tokens in WebUi.Style.portable_keys()
    assert :state_variants in WebUi.Style.widget_style_hooks()
  end
  ...
  test "theme catalog exposes defaults, palette roles, and token resolution" do
    assert [:default, :midnight] = WebUi.Theme.catalog_ids() |> Enum.sort()
    assert :canvas in WebUi.Theme.palette_roles()
    assert WebUi.Theme.default_theme().id == :default
    assert WebUi.Theme.runtime_contract().authoritative_server_theme

    assert {:ok, %{surface: :panel}} = WebUi.Theme.resolve_token(:default, [:surface, :default])

    assert {:ok, %{variant: :primary, tone: :accent}} =
             WebUi.Theme.resolve_token(:default, [:button, :primary])

    assert {:error, :unknown_token} = WebUi.Theme.resolve_token(:default, [:missing, :token])
  end
end
