defmodule WebUi.PhaseSixIntegrationTest do
  ...
  test "preview, inspection, export, and validation workflows remain coherent across the split runtime" do
    inspect_catalog = WebUi.Inspect.catalog()
    assert {:ok, export_artifact} = WebUi.Export.artifact(:styling_continuity)
    assert {:ok, release_readiness} = WebUi.Validate.release_readiness(:summary)

    assert inspect_catalog.package_overview.runtime.capabilities != []
    assert WebUi.Inspect in inspect_catalog.preview_surfaces

    assert export_artifact.artifact_names.comparison ==
             "web_ui.examples.styling_continuity.comparison"
  ...
  test "strict release-readiness modes fail deterministically on missing coverage or stale diagnostics" do
    ...
    assert {:error, failure_report} =
             WebUi.Validate.release_readiness(
               :strict,
