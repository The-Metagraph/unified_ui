defmodule WebUi.InspectionTest do
  ...
  test "runtime inspection exposes server resolved style state and frontend realization state" do
    screen =
      WebUi.Widgets.screen(
        "inspection-surface",
        "Inspection Surface",
        [
          WebUi.Widgets.button("save-button", "Save",
            variant: :primary,
    ...
    assert {:ok, snapshot} =
             WebUi.Inspection.runtime_snapshot(state, %{focused_id: "save-button"})
  ...
  test "continuity diagnostics stay quiet for aligned inputs and actionable for drift" do
    ...
    assert {:ok, passing_report} =
             WebUi.Continuity.compare(native_foundational_state, canonical_foundational_state)
    ...
    assert {:ok, native_drift_state} = WebUi.Runtime.mount_native_screen(native_drift_screen)
    assert {:ok, canonical_drift_state} = WebUi.Runtime.mount_iur_screen(canonical_drift_element)

    assert {:ok, failing_report} =
             WebUi.Continuity.compare(native_drift_state, canonical_drift_state)
