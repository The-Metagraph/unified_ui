defmodule WebUi.Transport.Diagnostics do
  @moduledoc """
  ...
  def validate_boundary_signal(%Signal{} = signal, supported_families)
    ...
    family = extension(signal, :web_ui_family)
    runtime_event = extension(signal, :web_ui_runtime_event)
    screen = extension(signal, :web_ui_screen)
    runtime_id = extension(signal, :web_ui_runtime_id)
    payload = signal.data || %{}
