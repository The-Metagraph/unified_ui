defmodule WebUi.Continuity do
  @moduledoc """
  Native-versus-canonical continuity diagnostics for `web_ui`.
  """

  alias WebUi.Inspection
  ...
  defp widget_identity_diagnostic(id, native_node, canonical_node) do
    if native_node.kind == canonical_node.kind do
      ...
      diagnostic(
        ...
        :kind_mismatch,
        id,
        "Inspect WebUi.Renderer for native/canonical widget identity drift",
        native_node.kind,
        canonical_node.kind
      )
    end
    ...
    cond do
      is_nil(native_node.theme) or is_nil(canonical_node.theme) ->
        diagnostic(
          ...
          :missing_theme,
          id,
          "Inspect WebUi.ServerRuntime.StyleResolver theme propagation into the render payload",
          native_node.theme,
      ...
      true ->
        diagnostic(
          ...
          :theme_mismatch,
          id,
          "Inspect WebUi.ServerRuntime.StyleResolver and renderer theme defaults for propagation drift",
          native_node.theme,
          canonical_node.theme
        )
    end
  ...
  defp resolved_style_diagnostic(id, native_node, canonical_node) do
    if native_node.resolved_styles == canonical_node.resolved_styles do
      ...
      diagnostic(
        ...
        :resolved_style_mismatch,
        id,
        "Inspect WebUi.ServerRuntime.StyleResolver and canonical style mapping for resolved-style drift",
  ...
  defp frontend_realization_diagnostic(id, native_node, canonical_node) do
    ...
    if native_projection == canonical_projection do
      ...
      diagnostic(
        ...
        :frontend_realization_drift,
        id,
        "Inspect WebUi.FrontendRuntime.Realization and StyleRealization for browser-facing drift",
        native_projection,
        canonical_projection
      )
    end
