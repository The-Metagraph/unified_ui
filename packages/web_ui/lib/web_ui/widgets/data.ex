defmodule WebUi.Widgets.Data do
  @moduledoc """
  Advanced data and document-oriented widgets for direct-use `web_ui` review
  flows.
  """
  ...
  def table(columns, rows, opts \\ []) when is_list(columns) and is_list(rows) do
    ...
    Builder.widget(:table,
      ...
      events:
        Builder.events(opts, change: :change, sort: :sort, filter: :filter, paginate: :paginate),
      metadata: Builder.metadata(opts, %{native_surface: :data, review_surface?: true})
    )
  end

  @spec tree_view([keyword() | map()], keyword() | map()) :: WebUi.Widget.t()
  def tree_view(nodes, opts \\ []) when is_list(nodes) do
    opts = Builder.options(opts)

    Builder.widget(:tree_view,
      id: Builder.require_id!(opts, :tree_view),
      props: %{
      ...
      state: Builder.state(opts, [:disabled?, :focused?, :loading?]),
      style_hooks: Builder.style_hooks(opts),
      events: Builder.events(opts, change: :change, filter: :filter, expand: :expand),
      metadata: Builder.metadata(opts, %{native_surface: :data, review_surface?: true})
    )
  end

  @spec markdown_viewer(String.t(), keyword() | map()) :: WebUi.Widget.t()
  def markdown_viewer(markdown, opts \\ []) when is_binary(markdown) do
    opts = Builder.options(opts)

    Builder.widget(:markdown_viewer,
      id: Builder.require_id!(opts, :markdown_viewer),
      family: :document,
      props: %{
        source: markdown,
        format: :markdown,
  ...
  @spec log_viewer([keyword() | map()], keyword() | map()) :: WebUi.Widget.t()
  def log_viewer(entries, opts \\ []) when is_list(entries) do
    opts = Builder.options(opts)

    Builder.widget(:log_viewer,
      id: Builder.require_id!(opts, :log_viewer),
      family: :document,
      props: %{
      ...
      style_hooks: Builder.style_hooks(opts),
      events: Builder.events(opts, filter: :filter, paginate: :paginate),
      metadata: Builder.metadata(opts, %{native_surface: :document, review_surface?: true})
    )
  end
