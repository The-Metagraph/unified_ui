defmodule WebUi.Widgets.Layout do
  ...
  @spec scroll_bar(String.t() | atom(), keyword() | map()) :: WebUi.Widget.t()
  ...
  defp validate_viewport_opts!(opts) do
    if Builder.option(opts, :independent_scroll, false) and
      ...
      raise ArgumentError,
            "web_ui viewport widgets cannot declare :independent_scroll and :sync_group together"
    end
  end
  ...
  defp validate_scroll_bar_opts!(opts) do
    if is_nil(Builder.option(opts, :viewport_ref)) and is_nil(Builder.option(opts, :sync_group)) do
      raise ArgumentError,
            "web_ui scroll_bar widgets require either :viewport_ref or :sync_group"
    end
  end
  ...
  defp validate_ratio!(_ratio) do
    raise ArgumentError, "web_ui split_pane widgets require a :ratio between 0 and 1"
  end
  ...
  defp validate_split_scroll_sync!(primary, secondary, opts) do
    if Builder.option(opts, :sync_scroll, false) and
      ...
      raise ArgumentError,
            "web_ui split_pane widgets require both panes to be :viewport widgets when :sync_scroll is true"
    end
  end
  ...
  defp normalize_offset(_value) do
    raise ArgumentError,
          "web_ui viewport widgets require :offset to be an integer, {x, y}, or map-like coordinates"
  end
  ...
  defp normalize_position(_value) do
    raise ArgumentError,
          "web_ui scroll_bar widgets require :position to be numeric, {start, end}, or map-like"
  end
