defmodule WebUi.Widgets.Builder do
  @moduledoc false
  ...
  def require_id!(opts, kind) do
    case option(opts, :id) do
      nil -> raise ArgumentError, "web_ui #{inspect(kind)} widgets require an :id"
      id -> id
    end
  end
  ...
  def child!(input) when is_map(input) or is_list(input) do
    case Widgets.normalize(input) do
      {:ok, widget} -> widget
      {:error, reason} -> raise ArgumentError, "invalid nested web_ui widget: #{inspect(reason)}"
    end
  end
  ...
  def children!(children) when is_list(children) do
    case Widgets.normalize_many(children) do
      {:ok, widgets} -> widgets
      {:error, reason} -> raise ArgumentError, "invalid nested web_ui widgets: #{inspect(reason)}"
    end
  end
  ...
  @spec style_hooks(map()) :: [atom() | String.t()]
  def style_hooks(opts) do
    opts
    |> option(:style_hooks, [])
    |> List.wrap()
