defmodule LiveUi.Layout.Grid do
  ...
  defp browser_attrs(assigns) do
    ...
    |> Map.merge(grid_css_vars(assigns.columns, assigns.rows))
    |> BrowserAttrs.from_css_vars()
  end

  defp grid_css_vars(nil, nil), do: %{}

  defp grid_css_vars(columns, rows) do
    %{}
    |> maybe_put("--live-ui-grid-columns", columns)
    |> maybe_put("--live-ui-grid-rows", rows)
  end
