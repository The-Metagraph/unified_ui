defmodule LiveUi.Demo.Style do
  ...
  @spec link(String.t() | nil, keyword()) :: map()
  def link(class \\ nil, opts \\ []) do
    LiveUi.Style.component_assigns(:link,
      theme: @theme,
      tone: Keyword.get(opts, :tone),
      variant: Keyword.get(opts, :variant),
      state: Keyword.get(opts, :state),
      class: merge_classes(["live-ui-demo-link", class])
    )
  end
  ...
  @spec direct(atom(), keyword()) :: map()
  def direct(component, opts \\ []) do
    LiveUi.Style.component_assigns(component,
      theme: @theme,
      tone: Keyword.get(opts, :tone),
      variant: Keyword.get(opts, :variant),
