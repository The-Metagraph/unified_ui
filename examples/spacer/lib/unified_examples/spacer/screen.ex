defmodule UnifiedExamples.Spacer.Screen do
  @moduledoc """
  Self-contained spacer proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.Spacer.Helpers
  alias UnifiedExamples.Spacer.StyleProfile
  alias UnifiedExamples.Spacer.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :spacer_example_screen])
    tags([:example, :spacer])
  end
  ...
  signals do
    ...
    interaction do
      id(:spacer_example_screen_interaction)
      family(:click)
      intent(:inspect_spacer)

      source_context(
        element_id: :spacer_example_screen_interaction_trigger,
        scope: :screen
      )
      ...
      payload_mapping(
        widget: :spacer,
        example: :spacer,
        outcome: "The spacer example should make invisible spacing choices easier to review through the shared interaction story.",
        source: :shared_example_trigger
      )

      summary("Use the shared trigger to show how the spacer example explains authored rhythm and spacing.")
    end
  end
  ...
  composition do
    root(:spacer_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :spacer_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :spacer_example_screen_title do
        value("Spacer Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :spacer_example_screen_summary do
      ...
      box :spacer_example_screen_panel do
        theme_ref(Theme.default_theme_id())
      ...
      button :spacer_example_screen_interaction_trigger do
        label("Highlight the spacing story")
        interaction_refs([:spacer_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
      ...
      text :spacer_example_screen_notes_text do
        value("Spacer examples keep the local shell while foregrounding one primary spacer widget.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.Spacer.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    idle_prompt: "Use the shared trigger to show how the spacer example explains authored rhythm and spacing.",
    outcome: "The spacer example should make invisible spacing choices easier to review through the shared interaction story.",
    target_surface: "spacer review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "Spacer examples keep the local shell while foregrounding one primary spacer widget.",
    widget: :spacer,
...
defmodule UnifiedExamples.Spacer.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.Spacer.StyleProfile do
  @moduledoc false
