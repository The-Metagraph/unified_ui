defmodule UnifiedExamples.StreamWidget.Screen do
  ...
  identity do
    ...
    authored_ref([:examples, :stream_widget_example_screen])
    tags([:example, :stream_widget])
  end
  ...
  signals do
    ...
    interaction do
      id(:stream_widget_example_screen_interaction)
      family(:command)
      intent(:inspect_stream_widget)

      source_context(
        element_id: :stream_widget_example_screen_interaction_trigger,
        scope: :screen
      )
  ...
  composition do
    root(:stream_widget_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :stream_widget_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :stream_widget_example_screen_title do
        value("Stream Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :stream_widget_example_screen_summary do
      ...
      box :stream_widget_example_screen_panel do
        theme_ref(Theme.default_theme_id())
      ...
      button :stream_widget_example_screen_interaction_trigger do
        label("Inspect the stream widget monitoring story")
        interaction_refs([:stream_widget_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
