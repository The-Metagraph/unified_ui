defmodule UnifiedExamples.Content.Screen do
  @moduledoc """
  Self-contained content proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.Content.Helpers
  alias UnifiedExamples.Content.StyleProfile
  alias UnifiedExamples.Content.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :content_example_screen])
  ...
  signals do
    ...
    interaction do
      id(:content_example_screen_interaction)
      family(:click)
      intent(:inspect_content)

      source_context(
        element_id: :content_example_screen_interaction_trigger,
        scope: :screen
      )
      ...
      payload_mapping(
        widget: :content,
        example: :content,
        outcome: "The content example should make grouping and semantic containment obvious even before reading the underlying DSL.",
        source: :shared_example_trigger
      )

      summary("Use the shared trigger to spotlight how the content container groups authored children into one reviewable story.")
    end
  end
  ...
  composition do
    root(:content_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :content_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :content_example_screen_title do
        value("Content Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :content_example_screen_summary do
      ...
      box :content_example_screen_panel do
        theme_ref(Theme.default_theme_id())
        ...
        UnifiedUi.Dsl.Composition.Box.Composition.Children.Content.content :content_example_primary_content do
          ...
          text :content_example_heading do
            value("Self-contained content block")
            theme_ref(Theme.default_theme_id())
            tone(:accent)
            variant(:headline)
          end
      ...
      button :content_example_screen_interaction_trigger do
        label("Review the content story")
        interaction_refs([:content_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
      ...
      text :content_example_screen_notes_text do
        value("Content examples keep the local shell while foregrounding one primary content container.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.Content.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    idle_prompt: "Use the shared trigger to spotlight how the content container groups authored children into one reviewable story.",
    outcome: "The content example should make grouping and semantic containment obvious even before reading the underlying DSL.",
    target_surface: "content review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "Content examples keep the local shell while foregrounding one primary content container.",
    widget: :content,
...
defmodule UnifiedExamples.Content.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.Content.StyleProfile do
  @moduledoc false
