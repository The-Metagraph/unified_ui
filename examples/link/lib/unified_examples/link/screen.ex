defmodule UnifiedExamples.Link.Screen do
  @moduledoc """
  Self-contained link proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.Link.Helpers
  alias UnifiedExamples.Link.StyleProfile
  alias UnifiedExamples.Link.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :link_example_screen])
    tags([:example, :link])
  end
  ...
  signals do
    ...
    interaction do
      id(:link_example_screen_interaction)
      family(:click)
      intent(:inspect_link)

      source_context(
        element_id: :link_example_screen_interaction_trigger,
        scope: :screen
      )
      ...
      payload_mapping(
        widget: :link,
        example: :link,
        outcome: "The link example should communicate the authored destination and make the navigation intent obvious before anyone follows it.",
        source: :shared_example_trigger
      )

      summary("Use the shared trigger to review how the link example explains its authored destination before navigation.")
    end
  end
  ...
  composition do
    root(:link_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :link_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :link_example_screen_title do
        value("Link Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :link_example_screen_summary do
      ...
      box :link_example_screen_panel do
        theme_ref(Theme.default_theme_id())
        ...
        link :link_example_primary_link do
          label("Open the example documentation")
      ...
      button :link_example_screen_interaction_trigger do
        label("Review the link story")
        interaction_refs([:link_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
      ...
      text :link_example_screen_notes_text do
        value("Link examples keep the local shell while foregrounding one primary link widget.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.Link.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    idle_prompt: "Use the shared trigger to review how the link example explains its authored destination before navigation.",
    outcome: "The link example should communicate the authored destination and make the navigation intent obvious before anyone follows it.",
    target_surface: "link review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "Link examples keep the local shell while foregrounding one primary link widget.",
    widget: :link,
...
defmodule UnifiedExamples.Link.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.Link.StyleProfile do
  @moduledoc false
