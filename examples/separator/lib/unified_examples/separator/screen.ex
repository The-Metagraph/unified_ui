defmodule UnifiedExamples.Separator.Screen do
  @moduledoc """
  Self-contained separator proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.Separator.Helpers
  alias UnifiedExamples.Separator.StyleProfile
  alias UnifiedExamples.Separator.Theme
  ...
  identity do
    ...
    description("Focused content-oriented example using the local example shell")
    authored_ref([:examples, :separator_example_screen])
    tags([:example, :separator])
  end
  ...
  signals do
    ...
    interaction do
      id(:separator_example_screen_interaction)
      family(:click)
      intent(:inspect_separator)

      source_context(
        element_id: :separator_example_screen_interaction_trigger,
        scope: :screen
      )
      ...
      payload_mapping(
        widget: :separator,
        example: :separator,
        outcome: "The separator example should explain the authored visual boundary and why it matters in the surrounding composition.",
        source: :shared_example_trigger
      )

      summary("Use the shared trigger to call out how the separator organizes the reviewed content.")
    end
  end
  ...
  composition do
    root(:separator_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :separator_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :separator_example_screen_title do
        value("Separator Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :separator_example_screen_summary do
        value("Focused content-oriented example using the local example shell")
        theme_ref(Theme.default_theme_id())
      ...
      box :separator_example_screen_panel do
        theme_ref(Theme.default_theme_id())
      ...
      button :separator_example_screen_interaction_trigger do
        label("Highlight the separator story")
        interaction_refs([:separator_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
      ...
      text :separator_example_screen_notes_text do
        value("Separator examples keep the local shell while foregrounding one primary separator widget.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.Separator.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    idle_prompt: "Use the shared trigger to call out how the separator organizes the reviewed content.",
    outcome: "The separator example should explain the authored visual boundary and why it matters in the surrounding composition.",
    target_surface: "separator review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "Separator examples keep the local shell while foregrounding one primary separator widget.",
    widget: :separator,
...
defmodule UnifiedExamples.Separator.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.Separator.StyleProfile do
  @moduledoc false
