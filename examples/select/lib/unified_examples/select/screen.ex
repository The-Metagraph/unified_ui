defmodule UnifiedExamples.Select.Screen do
  @moduledoc """
  Self-contained select proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.Select.Helpers
  alias UnifiedExamples.Select.StyleProfile
  alias UnifiedExamples.Select.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :select_example_screen])
    tags([:example, :select])
  end
  ...
  signals do
    ...
    interaction do
      id(:select_example_screen_interaction)
      family(:selection)
      intent(:inspect_select)

      source_context(
        element_id: :select_example_screen_shell,
        scope: :screen
      )
      ...
      payload_mapping(
        widget: :select,
        example: :select,
        outcome: "The review panel should explain how the select example turns live form input into a browser-visible selection outcome.",
        source: :shared_form_shell
      )

      summary("Interact with the select example to see how its authored selection signal updates the shared review story.")
    end
  end
  ...
  composition do
    root(:select_example_screen_root)
    mode(:screen)
    summary("Self-contained form-oriented example-app shell")

    form_builder :select_example_screen_shell do
      summary("Self-contained form-oriented example-app shell")
      theme_ref(Theme.default_theme_id())
      style_refs([:example_form_shell])
      tone(:surface)
      variant(:panel)
      interaction_refs([:select_example_screen_interaction])

      text :select_example_screen_title do
        value("Select Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :select_example_screen_summary do
      ...
      spacer :select_example_screen_form_gap do
        size(:sm)
      end
      ...
      text :select_example_screen_notes_text do
        value("Select examples keep the local form shell while foregrounding one menu-based choice control.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.Select.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    trigger_label: nil,
    idle_prompt: "Interact with the select example to see how its authored selection signal updates the shared review story.",
    outcome: "The review panel should explain how the select example turns live form input into a browser-visible selection outcome.",
    target_surface: "select review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "Select examples keep the local form shell while foregrounding one menu-based choice control.",
    widget: :select,
...
defmodule UnifiedExamples.Select.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.Select.StyleProfile do
  @moduledoc false
