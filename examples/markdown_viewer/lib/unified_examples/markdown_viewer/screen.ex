defmodule UnifiedExamples.MarkdownViewer.Screen do
  @moduledoc """
  Self-contained markdown-viewer proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.MarkdownViewer.Helpers
  alias UnifiedExamples.MarkdownViewer.StyleProfile
  alias UnifiedExamples.MarkdownViewer.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :markdown_viewer_example_screen])
    tags([:example, :markdown_viewer])
  end
  ...
  signals do
    ...
    interaction do
      id(:markdown_viewer_example_screen_interaction)
      family(:focus)
      intent(:inspect_markdown_viewer)

      source_context(
        element_id: :markdown_viewer_example_screen_interaction_trigger,
        scope: :screen
      )
      ...
      payload_mapping(
        ...
        outcome:
          "The review panel should explain how the markdown viewer example turns an authored canonical interaction into a browser-visible data story reviewers can understand quickly.",
        source: :shared_example_trigger
      )

      summary(
        "Use the shared trigger to see how the markdown viewer example explains focus changes such as focus, filtering, or selection."
      )
    end
  end
  ...
  composition do
    root(:markdown_viewer_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :markdown_viewer_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :markdown_viewer_example_screen_title do
        value("Markdown Viewer Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :markdown_viewer_example_screen_summary do
      ...
      box :markdown_viewer_example_screen_panel do
        theme_ref(Theme.default_theme_id())
        ...
        markdown_viewer :markdown_viewer_example_primary_document do
          source(Helpers.markdown_source())
      ...
      button :markdown_viewer_example_screen_interaction_trigger do
        label("Inspect the markdown viewer data story")
        interaction_refs([:markdown_viewer_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
      ...
      text :markdown_viewer_example_screen_notes_text do
        value("Markdown viewer examples foreground one canonical document surface inside the local shell.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.MarkdownViewer.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    trigger_label: "Inspect the markdown viewer data story",
    idle_prompt:
      "Use the shared trigger to see how the markdown viewer example explains focus changes such as focus, filtering, or selection.",
    outcome:
      "The review panel should explain how the markdown viewer example turns an authored canonical interaction into a browser-visible data story reviewers can understand quickly.",
    target_surface: "markdown viewer review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "Markdown viewer examples foreground one canonical document surface inside the local shell.",
    widget: :markdown_viewer,
  ...
  @spec markdown_source() :: String.t()
  def markdown_source, do: @markdown_source
end

defmodule UnifiedExamples.MarkdownViewer.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.MarkdownViewer.StyleProfile do
  @moduledoc false
