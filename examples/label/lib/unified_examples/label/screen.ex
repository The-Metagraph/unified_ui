defmodule UnifiedExamples.Label.Screen do
  @moduledoc """
  Self-contained label proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.Label.Helpers
  alias UnifiedExamples.Label.StyleProfile
  alias UnifiedExamples.Label.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :label_example_screen])
    tags([:example, :label])
  end
  ...
  signals do
    ...
    interaction do
      id(:label_example_screen_interaction)
      family(:click)
      intent(:inspect_label)

      source_context(
        element_id: :label_example_screen_interaction_trigger,
        scope: :screen
      )
      ...
      payload_mapping(
        widget: :label,
        example: :label,
        outcome: "The label example should make the authored label relationship easy to understand in the browser without inspecting source.",
        source: :shared_example_trigger
      )

      summary("Use the shared trigger to call out how the label example frames authored relationships for reviewers.")
    end
  end
  ...
  composition do
    root(:label_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :label_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :label_example_screen_title do
        value("Label Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :label_example_screen_summary do
      ...
      box :label_example_screen_panel do
        theme_ref(Theme.default_theme_id())
      ...
      button :label_example_screen_interaction_trigger do
        label("Highlight the label relationship")
        interaction_refs([:label_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
      ...
      text :label_example_screen_notes_text do
        value("Label examples keep the local shell while foregrounding one primary label widget.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.Label.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    idle_prompt: "Use the shared trigger to call out how the label example frames authored relationships for reviewers.",
    outcome: "The label example should make the authored label relationship easy to understand in the browser without inspecting source.",
    target_surface: "label review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "Label examples keep the local shell while foregrounding one primary label widget.",
    widget: :label,
...
defmodule UnifiedExamples.Label.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.Label.StyleProfile do
  @moduledoc false
