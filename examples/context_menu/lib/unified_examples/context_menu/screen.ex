defmodule UnifiedExamples.ContextMenu.Screen do
  ...
  identity do
    ...
    authored_ref([:examples, :context_menu_example_screen])
    tags([:example, :context_menu])
  end
  ...
  signals do
    ...
    interaction do
      id(:context_menu_example_screen_interaction)
      family(:open)
      intent(:inspect_context_menu)

      source_context(
        element_id: :context_menu_example_screen_interaction_trigger,
        scope: :screen
      )
  ...
  composition do
    root(:context_menu_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :context_menu_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :context_menu_example_screen_title do
        value("Context Menu Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :context_menu_example_screen_summary do
      ...
      box :context_menu_example_screen_panel do
        theme_ref(Theme.default_theme_id())
        ...
        context_menu :context_menu_example_primary_context_menu do
          options(Helpers.context_menu_options())
      ...
      button :context_menu_example_screen_interaction_trigger do
        label("Inspect the context menu layered story")
        interaction_refs([:context_menu_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
