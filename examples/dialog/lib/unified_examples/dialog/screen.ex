...
defmodule UnifiedExamples.Dialog.Screen do
  ...
  identity do
    ...
    authored_ref([:examples, :dialog_example_screen])
    tags([:example, :dialog])
  end
  ...
  signals do
    ...
    interaction do
      id(:dialog_example_screen_interaction)
      family(:open)
      intent(:inspect_dialog)

      source_context(
        element_id: :dialog_example_screen_interaction_trigger,
        scope: :screen
      )
  ...
  composition do
    root(:dialog_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :dialog_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :dialog_example_screen_title do
        value("Dialog Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :dialog_example_screen_summary do
      ...
      box :dialog_example_screen_panel do
        theme_ref(Theme.default_theme_id())
        ...
        button :dialog_example_open_button do
          label(Helpers.dialog_trigger_label())
          theme_ref(Theme.default_theme_id())
          tone(:accent)
          variant(:quiet)
        end
      ...
      button :dialog_example_screen_interaction_trigger do
        label("Inspect the dialog layered story")
        interaction_refs([:dialog_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
      ...
      text :dialog_example_screen_notes_text do
        value("Dialog examples foreground one canonical modal surface inside the local shell.")
        theme_ref(Theme.default_theme_id())
