defmodule UnifiedExamples.Field.Screen do
  @moduledoc """
  Self-contained field proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.Field.Helpers
  alias UnifiedExamples.Field.StyleProfile
  alias UnifiedExamples.Field.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :field_example_screen])
    tags([:example, :field])
  end
  ...
  signals do
    ...
    interaction do
      id(:field_example_screen_interaction)
      family(:change)
      intent(:inspect_field)

      source_context(
        element_id: :field_example_screen_shell,
        scope: :screen
      )
      ...
      payload_mapping(
        widget: :field,
        example: :field,
        outcome: "The review panel should explain how the field example turns live form input into a browser-visible change outcome.",
        source: :shared_form_shell
      )

      summary("Interact with the field example to see how its authored change signal updates the shared review story.")
    end
  end
  ...
  composition do
    root(:field_example_screen_root)
    mode(:screen)
    summary("Self-contained form-oriented example-app shell")

    form_builder :field_example_screen_shell do
      summary("Self-contained form-oriented example-app shell")
      theme_ref(Theme.default_theme_id())
      style_refs([:example_form_shell])
      tone(:surface)
      variant(:panel)
      interaction_refs([:field_example_screen_interaction])

      text :field_example_screen_title do
        value("Field Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :field_example_screen_summary do
      ...
      spacer :field_example_screen_form_gap do
        size(:sm)
      end
      ...
      text :field_example_screen_notes_text do
        value("Field examples keep the local form shell while foregrounding one primary labeled field.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.Field.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    trigger_label: nil,
    idle_prompt: "Interact with the field example to see how its authored change signal updates the shared review story.",
    outcome: "The review panel should explain how the field example turns live form input into a browser-visible change outcome.",
    target_surface: "field review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "Field examples keep the local form shell while foregrounding one primary labeled field.",
    widget: :field,
...
defmodule UnifiedExamples.Field.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.Field.StyleProfile do
  @moduledoc false
