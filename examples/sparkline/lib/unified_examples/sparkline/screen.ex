defmodule UnifiedExamples.Sparkline.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    trigger_label: "Inspect the sparkline feedback story",
    idle_prompt:
    ...
    target_surface: "sparkline review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "Sparkline examples foreground one canonical trend line inside the local shell.",
    widget: :sparkline,
    theme_id: @default_theme_id,
    interaction_demo: @interaction_demo
  }

  @sparkline_points [34, 41, 39, 52, 48, 55]
  ...
  @spec sparkline_points() :: [integer()]
  def sparkline_points, do: @sparkline_points
end

defmodule UnifiedExamples.Sparkline.Theme do
  @moduledoc false

  alias UnifiedExamples.Sparkline.Helpers
...
defmodule UnifiedExamples.Sparkline.StyleProfile do
  @moduledoc false
...
defmodule UnifiedExamples.Sparkline.Screen do
  @moduledoc """
  Self-contained sparkline proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.Sparkline.Helpers
  alias UnifiedExamples.Sparkline.StyleProfile
  alias UnifiedExamples.Sparkline.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :sparkline_example_screen])
    tags([:example, :sparkline])
  end
  ...
  signals do
    ...
    interaction do
      id(:sparkline_example_screen_interaction)
      family(:click)
      intent(:inspect_sparkline)

      source_context(
        element_id: :sparkline_example_screen_interaction_trigger,
        scope: :screen
      )
  ...
  composition do
    root(:sparkline_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :sparkline_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :sparkline_example_screen_title do
        value("Sparkline Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :sparkline_example_screen_summary do
      ...
      box :sparkline_example_screen_panel do
        theme_ref(Theme.default_theme_id())
        ...
        sparkline :sparkline_example_primary_chart do
          points(Helpers.sparkline_points())
          theme_ref(Theme.default_theme_id())
          tone(:surface)
          variant(:quiet)
        end
      ...
      button :sparkline_example_screen_interaction_trigger do
        label("Inspect the sparkline feedback story")
        interaction_refs([:sparkline_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
      ...
      text :sparkline_example_screen_notes_text do
        value("Sparkline examples foreground one canonical trend line inside the local shell.")
        theme_ref(Theme.default_theme_id())
