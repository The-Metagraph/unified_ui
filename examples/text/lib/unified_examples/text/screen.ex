defmodule UnifiedExamples.Text.Screen do
  @moduledoc """
  Self-contained text proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.Text.Helpers
  alias UnifiedExamples.Text.StyleProfile
  alias UnifiedExamples.Text.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :text_example_screen])
    tags([:example, :text])
  end
  ...
  signals do
    ...
    interaction do
      id(:text_example_screen_interaction)
      family(:click)
      intent(:inspect_text)

      source_context(
        element_id: :text_example_screen_interaction_trigger,
        scope: :screen
      )
  ...
  composition do
    root(:text_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :text_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :text_example_screen_title do
        value("Text Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :text_example_screen_summary do
      ...
      box :text_example_screen_panel do
        theme_ref(Theme.default_theme_id())
        ...
        text :text_example_primary_content do
          value("Self-contained text example")
          theme_ref(Theme.default_theme_id())
          tone(:accent)
          variant(:headline)
        end
      ...
      button :text_example_screen_interaction_trigger do
        label("Highlight the text story")
        interaction_refs([:text_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
      ...
      text :text_example_screen_notes_text do
        value("Text examples keep the local shell while foregrounding one primary content widget.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.Text.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    outcome:
      "The text example should spotlight its authored copy and explain why the shared trigger exists for otherwise passive content.",
    target_surface: "text review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "Text examples keep the local shell while foregrounding one primary content widget.",
    widget: :text,
...
defmodule UnifiedExamples.Text.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.Text.StyleProfile do
  @moduledoc false
