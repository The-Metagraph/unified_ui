defmodule UnifiedExamples.TimeInput.Screen do
  @moduledoc """
  Self-contained time_input proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.TimeInput.Helpers
  alias UnifiedExamples.TimeInput.StyleProfile
  alias UnifiedExamples.TimeInput.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :time_input_example_screen])
    tags([:example, :time_input])
  end
  ...
  signals do
    ...
    interaction do
      id(:time_input_example_screen_interaction)
      family(:change)
      intent(:inspect_time_input)

      source_context(
        element_id: :time_input_example_screen_shell,
        scope: :screen
      )
      ...
      payload_mapping(
        widget: :time_input,
        example: :time_input,
        outcome: "The review panel should explain how the time input example turns live form input into a browser-visible change outcome.",
        source: :shared_form_shell
      )

      summary("Interact with the time input example to see how its authored change signal updates the shared review story.")
    end
  end
  ...
  composition do
    root(:time_input_example_screen_root)
    mode(:screen)
    summary("Self-contained form-oriented example-app shell")

    form_builder :time_input_example_screen_shell do
      summary("Self-contained form-oriented example-app shell")
      theme_ref(Theme.default_theme_id())
      style_refs([:example_form_shell])
      tone(:surface)
      variant(:panel)
      interaction_refs([:time_input_example_screen_interaction])

      text :time_input_example_screen_title do
        value("Time Input Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :time_input_example_screen_summary do
      ...
      spacer :time_input_example_screen_form_gap do
        size(:sm)
      end
      ...
      text :time_input_example_screen_notes_text do
        value("Time input examples keep the local form shell while foregrounding one time control.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.TimeInput.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    trigger_label: nil,
    idle_prompt: "Interact with the time input example to see how its authored change signal updates the shared review story.",
    outcome: "The review panel should explain how the time input example turns live form input into a browser-visible change outcome.",
    target_surface: "time_input review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "Time input examples keep the local form shell while foregrounding one time control.",
    widget: :time_input,
...
defmodule UnifiedExamples.TimeInput.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.TimeInput.StyleProfile do
  @moduledoc false
