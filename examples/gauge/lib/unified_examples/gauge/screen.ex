defmodule UnifiedExamples.Gauge.Screen do
  ...
  identity do
    ...
    authored_ref([:examples, :gauge_example_screen])
    tags([:example, :gauge])
  end
  ...
  signals do
    ...
    interaction do
      id(:gauge_example_screen_interaction)
      family(:click)
      intent(:inspect_gauge)

      source_context(
        element_id: :gauge_example_screen_interaction_trigger,
        scope: :screen
      )
  ...
  composition do
    root(:gauge_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :gauge_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :gauge_example_screen_title do
        value("Gauge Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :gauge_example_screen_summary do
      ...
      box :gauge_example_screen_panel do
        theme_ref(Theme.default_theme_id())
        ...
        gauge :gauge_example_primary_gauge do
          current(Helpers.gauge_current())
          minimum(Helpers.gauge_minimum())
          maximum(Helpers.gauge_maximum())
          label(Helpers.gauge_label())
          severity(Helpers.gauge_severity())
          status(Helpers.gauge_status())
          theme_ref(Theme.default_theme_id())
          tone(:surface)
          variant(:quiet)
        end
      ...
      button :gauge_example_screen_interaction_trigger do
        label("Inspect the gauge feedback story")
        interaction_refs([:gauge_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
