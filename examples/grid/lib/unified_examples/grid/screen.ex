defmodule UnifiedExamples.Grid.Screen do
  @moduledoc """
  Self-contained grid proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.Grid.Helpers
  alias UnifiedExamples.Grid.StyleProfile
  alias UnifiedExamples.Grid.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :grid_example_screen])
    tags([:example, :grid])
  end
  ...
  signals do
    ...
    interaction do
      id(:grid_example_screen_interaction)
      family(:click)
      intent(:inspect_grid)

      source_context(
        element_id: :grid_example_screen_interaction_trigger,
        scope: :screen
      )
      ...
      payload_mapping(
        ...
        example: :grid,
        outcome:
          "The grid example should make multi-cell structure easy to parse without losing the suite styling language.",
        source: :shared_example_trigger
      )

      summary(
        "Use the shared trigger to show how the grid example groups related values into one structured surface."
      )
    end
  end
  ...
  composition do
    root(:grid_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :grid_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :grid_example_screen_title do
        value("Grid Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :grid_example_screen_summary do
      ...
      box :grid_example_screen_panel do
        theme_ref(Theme.default_theme_id())
      ...
      button :grid_example_screen_interaction_trigger do
        label("Review the grid layout story")
        interaction_refs([:grid_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
      ...
      text :grid_example_screen_notes_text do
        value("Grid examples keep the local shell while foregrounding one multi-cell layout surface.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.Grid.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    trigger_label: "Review the grid layout story",
    idle_prompt:
      "Use the shared trigger to show how the grid example groups related values into one structured surface.",
    outcome:
      "The grid example should make multi-cell structure easy to parse without losing the suite styling language.",
    target_surface: "grid review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "Grid examples keep the local shell while foregrounding one multi-cell layout surface.",
    widget: :grid,
...
defmodule UnifiedExamples.Grid.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.Grid.StyleProfile do
  @moduledoc false
