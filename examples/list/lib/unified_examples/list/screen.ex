defmodule UnifiedExamples.List.Screen do
  @moduledoc """
  Self-contained list proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.List.Helpers
  alias UnifiedExamples.List.StyleProfile
  alias UnifiedExamples.List.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :list_example_screen])
    tags([:example, :list])
  end
  ...
  signals do
    ...
    interaction do
      id(:list_example_screen_interaction)
      family(:selection)
      intent(:inspect_list)

      source_context(
        element_id: :list_example_screen_interaction_trigger,
        scope: :screen
      )
      ...
      payload_mapping(
        ...
        example: :list,
        outcome:
          "The review panel should explain how the list example turns an authored canonical interaction into a browser-visible data story reviewers can understand quickly.",
        source: :shared_example_trigger
      )

      summary(
        "Use the shared trigger to see how the list example explains selection changes such as focus, filtering, or selection."
      )
    end
  end
  ...
  composition do
    root(:list_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :list_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :list_example_screen_title do
        value("List Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :list_example_screen_summary do
      ...
      box :list_example_screen_panel do
        theme_ref(Theme.default_theme_id())
      ...
      button :list_example_screen_interaction_trigger do
        label("Inspect the list data story")
        interaction_refs([:list_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
      ...
      text :list_example_screen_notes_text do
        value("List examples foreground one canonical data list inside the local shell.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.List.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    trigger_label: "Inspect the list data story",
    idle_prompt:
      "Use the shared trigger to see how the list example explains selection changes such as focus, filtering, or selection.",
    outcome:
      "The review panel should explain how the list example turns an authored canonical interaction into a browser-visible data story reviewers can understand quickly.",
    target_surface: "list review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "List examples foreground one canonical data list inside the local shell.",
    widget: :list,
  ...
  @spec list_items() :: [keyword()]
  def list_items, do: @list_items
end

defmodule UnifiedExamples.List.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.List.StyleProfile do
  @moduledoc false
