defmodule UnifiedExamples.SupervisionTreeViewer.Screen do
  @moduledoc """
  Self-contained supervision-tree-viewer proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.SupervisionTreeViewer.Helpers
  alias UnifiedExamples.SupervisionTreeViewer.StyleProfile
  alias UnifiedExamples.SupervisionTreeViewer.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :supervision_tree_viewer_example_screen])
    tags([:example, :supervision_tree_viewer])
  end
  ...
  signals do
    ...
    interaction do
      id(:supervision_tree_viewer_example_screen_interaction)
      family(:command)
      intent(:inspect_supervision_tree_viewer)

      source_context(
        element_id: :supervision_tree_viewer_example_screen_interaction_trigger,
        scope: :screen
      )
      ...
      payload_mapping(
        ...
        example: :supervision_tree_viewer,
        outcome: "The review panel should explain how the supervision tree viewer example turns an authored canonical interaction into a browser-visible operational story.",
        source: :shared_example_trigger
      )

      summary(
        "Use the shared trigger to see how the supervision tree viewer example explains command changes for inspection, refresh, or focus flows."
      )
    end
  end
  ...
  composition do
    root(:supervision_tree_viewer_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :supervision_tree_viewer_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :supervision_tree_viewer_example_screen_title do
        value("Supervision Tree Viewer Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :supervision_tree_viewer_example_screen_summary do
      ...
      box :supervision_tree_viewer_example_screen_panel do
        theme_ref(Theme.default_theme_id())
        ...
        variant(:panel)
            ...
            supervision_tree_viewer :supervision_tree_viewer_example_primary_supervision_tree_viewer do
              topology(Helpers.topology())
              expanded?(Helpers.expanded?())
              theme_ref(Theme.default_theme_id())
              tone(:surface)
              variant(:quiet)
            end
      ...
      button :supervision_tree_viewer_example_screen_interaction_trigger do
        label("Inspect the supervision tree viewer monitoring story")
        interaction_refs([:supervision_tree_viewer_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
      ...
      text :supervision_tree_viewer_example_screen_notes_text do
        value("Supervision-tree examples foreground one canonical supervision hierarchy inside the local shell.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.SupervisionTreeViewer.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    trigger_label: "Inspect the supervision tree viewer monitoring story",
    idle_prompt: "Use the shared trigger to see how the supervision tree viewer example explains command changes for inspection, refresh, or focus flows.",
    outcome: "The review panel should explain how the supervision tree viewer example turns an authored canonical interaction into a browser-visible operational story.",
    target_surface: "supervision tree viewer review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "Supervision-tree examples foreground one canonical supervision hierarchy inside the local shell.",
    widget: :supervision_tree_viewer,
  ...
  @spec expanded?() :: boolean()
  def expanded?, do: @expanded
...
defmodule UnifiedExamples.SupervisionTreeViewer.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.SupervisionTreeViewer.StyleProfile do
  @moduledoc false
