defmodule UnifiedExamples.Tabs.Screen do
  @moduledoc """
  Self-contained tabs proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.Tabs.Helpers
  alias UnifiedExamples.Tabs.StyleProfile
  alias UnifiedExamples.Tabs.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :tabs_example_screen])
    tags([:example, :tabs])
  end
  ...
  signals do
    ...
    interaction do
      id(:tabs_example_screen_interaction)
      family(:navigation)
      intent(:inspect_tabs)

      source_context(
        element_id: :tabs_example_screen_interaction_trigger,
        scope: :screen
      )
      ...
      payload_mapping(
        ...
        example: :tabs,
        outcome:
          "The review panel should explain how the tabs example turns an authored canonical interaction into a browser-visible navigation story with clear active-state meaning.",
        source: :shared_example_trigger
      )

      summary(
        "Use the shared trigger to see how the tabs example explains navigation changes and active-state meaning."
      )
    end
  end
  ...
  composition do
    root(:tabs_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :tabs_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :tabs_example_screen_title do
        value("Tabs Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :tabs_example_screen_summary do
      ...
      box :tabs_example_screen_panel do
        theme_ref(Theme.default_theme_id())
      ...
      button :tabs_example_screen_interaction_trigger do
        label("Review the tabs navigation story")
        interaction_refs([:tabs_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
      ...
      text :tabs_example_screen_notes_text do
        value("Tabs examples keep the local shell while foregrounding one canonical view switcher.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.Tabs.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    trigger_label: "Review the tabs navigation story",
    idle_prompt:
      "Use the shared trigger to see how the tabs example explains navigation changes and active-state meaning.",
    outcome:
      "The review panel should explain how the tabs example turns an authored canonical interaction into a browser-visible navigation story with clear active-state meaning.",
    target_surface: "tabs review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "Tabs examples keep the local shell while foregrounding one canonical view switcher.",
    widget: :tabs,
...
defmodule UnifiedExamples.Tabs.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.Tabs.StyleProfile do
  @moduledoc false
