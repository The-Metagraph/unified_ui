defmodule UnifiedExamples.CommandPalette.Screen do
  @moduledoc """
  Self-contained command palette proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.CommandPalette.Helpers
  alias UnifiedExamples.CommandPalette.StyleProfile
  alias UnifiedExamples.CommandPalette.Theme
  ...
  identity do
    ...
    description("Focused navigation-oriented example using the local example shell")
    authored_ref([:examples, :command_palette_example_screen])
    tags([:example, :command_palette])
  end
  ...
  signals do
    ...
    interaction do
      id(:command_palette_example_screen_interaction)
      family(:command)
      intent(:inspect_command_palette)

      source_context(
        element_id: :command_palette_example_screen_interaction_trigger,
        scope: :screen
      )
      ...
      payload_mapping(
        ...
        example: :command_palette,
        outcome:
          "The review panel should explain how the command palette example turns an authored canonical interaction into a browser-visible command story reviewers can follow quickly.",
        source: :shared_example_trigger
      )

      summary(
        "Use the shared trigger to see how the command palette example explains command intent and the resulting reviewed command outcome."
      )
    end
  end
  ...
  composition do
    root(:command_palette_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :command_palette_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :command_palette_example_screen_title do
        value("Command Palette Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :command_palette_example_screen_summary do
        value("Focused navigation-oriented example using the local example shell")
        theme_ref(Theme.default_theme_id())
      ...
      box :command_palette_example_screen_panel do
        theme_ref(Theme.default_theme_id())
      ...
      button :command_palette_example_screen_interaction_trigger do
        label("Review the command palette command story")
        interaction_refs([:command_palette_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
      ...
      text :command_palette_example_screen_notes_text do
        value("Command palette examples foreground one canonical quick-action surface inside the local shell.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.CommandPalette.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    trigger_label: "Review the command palette command story",
    idle_prompt:
      "Use the shared trigger to see how the command palette example explains command intent and the resulting reviewed command outcome.",
    outcome:
      "The review panel should explain how the command palette example turns an authored canonical interaction into a browser-visible command story reviewers can follow quickly.",
    target_surface: "command palette review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    title: "Command Palette Widget Example",
    summary: "Focused navigation-oriented example using the local example shell",
    notes: "Command palette examples foreground one canonical quick-action surface inside the local shell.",
    widget: :command_palette,
...
defmodule UnifiedExamples.CommandPalette.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.CommandPalette.StyleProfile do
  @moduledoc false
