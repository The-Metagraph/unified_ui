...
defmodule UnifiedExamples.Icon.Screen do
  ...
  identity do
    ...
    authored_ref([:examples, :icon_example_screen])
    tags([:example, :icon])
  end
  ...
  signals do
    ...
    interaction do
      id(:icon_example_screen_interaction)
      family(:click)
      intent(:inspect_icon)

      source_context(
        element_id: :icon_example_screen_interaction_trigger,
        scope: :screen
      )
  ...
  composition do
    root(:icon_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :icon_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :icon_example_screen_title do
        value("Icon Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :icon_example_screen_summary do
      ...
      box :icon_example_screen_panel do
        theme_ref(Theme.default_theme_id())
      ...
      button :icon_example_screen_interaction_trigger do
        label("Highlight the icon story")
        interaction_refs([:icon_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
      ...
      text :icon_example_screen_notes_text do
        value("Icon examples keep the local shell while foregrounding one primary icon widget.")
        theme_ref(Theme.default_theme_id())
