defmodule UnifiedExamples.FileInput.Screen do
  @moduledoc """
  Self-contained file_input proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.FileInput.Helpers
  alias UnifiedExamples.FileInput.StyleProfile
  alias UnifiedExamples.FileInput.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :file_input_example_screen])
    tags([:example, :file_input])
  end
  ...
  signals do
    ...
    interaction do
      id(:file_input_example_screen_interaction)
      family(:change)
      intent(:inspect_file_input)

      source_context(
        element_id: :file_input_example_screen_shell,
        scope: :screen
      )
      ...
      payload_mapping(
        widget: :file_input,
        example: :file_input,
        outcome: "The review panel should explain how the file input example turns live form input into a browser-visible change outcome.",
        source: :shared_form_shell
      )

      summary("Interact with the file input example to see how its authored change signal updates the shared review story.")
    end
  end
  ...
  composition do
    root(:file_input_example_screen_root)
    mode(:screen)
    summary("Self-contained form-oriented example-app shell")

    form_builder :file_input_example_screen_shell do
      summary("Self-contained form-oriented example-app shell")
      theme_ref(Theme.default_theme_id())
      style_refs([:example_form_shell])
      tone(:surface)
      variant(:panel)
      interaction_refs([:file_input_example_screen_interaction])

      text :file_input_example_screen_title do
        value("File Input Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :file_input_example_screen_summary do
      ...
      spacer :file_input_example_screen_form_gap do
        size(:sm)
      end
      ...
      text :file_input_example_screen_notes_text do
        value("File input examples keep the local form shell while foregrounding one file picker control.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.FileInput.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    trigger_label: nil,
    idle_prompt: "Interact with the file input example to see how its authored change signal updates the shared review story.",
    outcome: "The review panel should explain how the file input example turns live form input into a browser-visible change outcome.",
    target_surface: "file_input review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "File input examples keep the local form shell while foregrounding one file picker control.",
    widget: :file_input,
...
defmodule UnifiedExamples.FileInput.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.FileInput.StyleProfile do
  @moduledoc false
