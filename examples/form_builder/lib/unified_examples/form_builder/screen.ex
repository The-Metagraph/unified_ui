defmodule UnifiedExamples.FormBuilder.Screen do
  @moduledoc """
  Self-contained form_builder proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.FormBuilder.Helpers
  alias UnifiedExamples.FormBuilder.StyleProfile
  alias UnifiedExamples.FormBuilder.Theme
  ...
  identity do
    ...
    description("Focused form-oriented example using the local example shell")
    authored_ref([:examples, :form_builder_example_screen])
    tags([:example, :form_builder])
  end
  ...
  signals do
    ...
    interaction do
      id(:form_builder_example_screen_interaction)
      family(:change)
      intent(:inspect_form_builder)

      source_context(
        element_id: :form_builder_example_screen_shell,
        scope: :screen
      )
      ...
      payload_mapping(
        widget: :form_builder,
        example: :form_builder,
        outcome: "The review panel should explain how the form builder example turns live form input into a browser-visible change outcome.",
        source: :shared_form_shell
      )

      summary("Interact with the form builder example to see how its authored change signal updates the shared review story.")
    end
  end
  ...
  composition do
    root(:form_builder_example_screen_root)
    mode(:screen)
    summary("Self-contained form-oriented example-app shell")

    form_builder :form_builder_example_screen_shell do
      summary("Self-contained form-oriented example-app shell")
      theme_ref(Theme.default_theme_id())
      style_refs([:example_form_shell])
      tone(:surface)
      variant(:panel)
      interaction_refs([:form_builder_example_screen_interaction])

      text :form_builder_example_screen_title do
        value("Form Builder Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :form_builder_example_screen_summary do
        value("Focused form-oriented example using the local example shell")
        theme_ref(Theme.default_theme_id())
      ...
      spacer :form_builder_example_screen_form_gap do
        size(:sm)
      end
      ...
      text :form_builder_example_screen_notes_text do
        value("Form builder examples keep the local form shell while foregrounding one primary form workflow.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.FormBuilder.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    trigger_label: nil,
    idle_prompt: "Interact with the form builder example to see how its authored change signal updates the shared review story.",
    outcome: "The review panel should explain how the form builder example turns live form input into a browser-visible change outcome.",
    target_surface: "form_builder review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    title: "Form Builder Example",
    summary: "Focused form-oriented example using the local example shell",
    notes: "Form builder examples keep the local form shell while foregrounding one primary form workflow.",
    widget: :form_builder,
...
defmodule UnifiedExamples.FormBuilder.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.FormBuilder.StyleProfile do
  @moduledoc false
