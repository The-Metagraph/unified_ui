defmodule UnifiedExamples.FieldGroup.Screen do
  @moduledoc """
  Self-contained field_group proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.FieldGroup.Helpers
  alias UnifiedExamples.FieldGroup.StyleProfile
  alias UnifiedExamples.FieldGroup.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :field_group_example_screen])
    tags([:example, :field_group])
  end
  ...
  signals do
    ...
    interaction do
      id(:field_group_example_screen_interaction)
      family(:change)
      intent(:inspect_field_group)

      source_context(
        element_id: :field_group_example_screen_shell,
        scope: :screen
      )
      ...
      payload_mapping(
        widget: :field_group,
        example: :field_group,
        outcome: "The review panel should explain how the field group example turns live form input into a browser-visible change outcome.",
        source: :shared_form_shell
      )

      summary("Interact with the field group example to see how its authored change signal updates the shared review story.")
    end
  end
  ...
  composition do
    root(:field_group_example_screen_root)
    mode(:screen)
    summary("Self-contained form-oriented example-app shell")

    form_builder :field_group_example_screen_shell do
      summary("Self-contained form-oriented example-app shell")
      theme_ref(Theme.default_theme_id())
      style_refs([:example_form_shell])
      tone(:surface)
      variant(:panel)
      interaction_refs([:field_group_example_screen_interaction])

      text :field_group_example_screen_title do
        value("Field Group Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :field_group_example_screen_summary do
      ...
      spacer :field_group_example_screen_form_gap do
        size(:sm)
      end
      ...
      text :field_group_example_screen_notes_text do
        value("Field group examples keep the local form shell while foregrounding one grouped set of fields.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.FieldGroup.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    trigger_label: nil,
    idle_prompt: "Interact with the field group example to see how its authored change signal updates the shared review story.",
    outcome: "The review panel should explain how the field group example turns live form input into a browser-visible change outcome.",
    target_surface: "field_group review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "Field group examples keep the local form shell while foregrounding one grouped set of fields.",
    widget: :field_group,
...
defmodule UnifiedExamples.FieldGroup.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.FieldGroup.StyleProfile do
  @moduledoc false
