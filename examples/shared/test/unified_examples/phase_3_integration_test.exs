defmodule UnifiedExamples.Phase3IntegrationTest do
  ...
  test "phase 3 apps are discoverable from the suite catalog and index and boot independently" do
  ...
  test "phase 3 apps preserve the shared DSL template and default theme across families" do
    Enum.each(@phase_entries, fn entry ->
      app = Catalog.app_module(entry.directory)
  ...
  test "phase 3 apps keep exactly one primary subject in the compiled tree" do
    Enum.each(@phase_entries, fn entry ->
      screen = Catalog.screen_module(entry.directory)
      shell = Runtime.renderable_element(Runtime.iur!(screen))
