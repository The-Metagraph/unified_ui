defmodule UnifiedExamples.Phase4IntegrationTest do
  ...
  test "phase 4 apps stay discoverable, self-contained, and independently bootable" do
  ...
  test "phase 4 apps preserve the self-contained shell, theme, and style baseline across advanced families" do
  ...
  test "phase 4 apps keep one primary subject and stay aligned with advanced catalog interaction contracts" do
    Enum.each(@phase_entries, fn entry ->
      screen = Catalog.screen_module(entry.directory)
      {:ok, metadata} = Tooling.review_metadata(entry.directory)
      shell = Runtime.renderable_element(Runtime.iur!(screen))
