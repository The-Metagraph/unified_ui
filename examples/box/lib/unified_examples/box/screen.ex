defmodule UnifiedExamples.Box.Screen do
  @moduledoc """
  Self-contained box proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.Box.Helpers
  alias UnifiedExamples.Box.StyleProfile
  alias UnifiedExamples.Box.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :box_example_screen])
    tags([:example, :box])
  end
  ...
  signals do
    ...
    interaction do
      id(:box_example_screen_interaction)
      family(:click)
      intent(:inspect_box)

      source_context(
        element_id: :box_example_screen_interaction_trigger,
        scope: :screen
      )
  ...
  composition do
    root(:box_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :box_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :box_example_screen_title do
        value("Box Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :box_example_screen_summary do
      ...
      box :box_example_screen_panel do
        theme_ref(Theme.default_theme_id())
        style_refs([:example_panel])
        tone(:surface)
        variant(:panel)

        box :box_example_primary_box do
          theme_ref(Theme.default_theme_id())
          style_refs([:example_panel])
          tone(:surface)
          variant(:panel)
          ...
          text :box_example_heading do
            value("Self-contained box container")
            theme_ref(Theme.default_theme_id())
            tone(:accent)
            variant(:headline)
          end
          ...
          text :box_example_body do
            value("Box widgets gather related content inside one local visual boundary.")
            theme_ref(Theme.default_theme_id())
            tone(:surface)
            variant(:body)
          end
      ...
      button :box_example_screen_interaction_trigger do
        label("Review the box layout story")
        interaction_refs([:box_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
      ...
      text :box_example_screen_notes_text do
        value("Box examples keep the local shell while foregrounding one primary layout container.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.Box.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    outcome:
      "The box example should make the authored layout container feel intentional and easy to review in the browser.",
    target_surface: "box review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "Box examples keep the local shell while foregrounding one primary layout container.",
    widget: :box,
...
defmodule UnifiedExamples.Box.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.Box.StyleProfile do
  @moduledoc false
