...
defmodule UnifiedExamples.Toast.Screen do
  ...
  identity do
    ...
    authored_ref([:examples, :toast_example_screen])
    tags([:example, :toast])
  end
  ...
  signals do
    ...
    interaction do
      id(:toast_example_screen_interaction)
      family(:open)
      intent(:inspect_toast)

      source_context(
        element_id: :toast_example_screen_interaction_trigger,
        scope: :screen
      )
  ...
  composition do
    root(:toast_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :toast_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :toast_example_screen_title do
        value("Toast Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :toast_example_screen_summary do
      ...
      box :toast_example_screen_panel do
        theme_ref(Theme.default_theme_id())
        ...
        button :toast_example_trigger do
          label(Helpers.toast_trigger_label())
          theme_ref(Theme.default_theme_id())
          tone(:accent)
          variant(:quiet)
        end
      ...
      button :toast_example_screen_interaction_trigger do
        label("Inspect the toast layered story")
        interaction_refs([:toast_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
