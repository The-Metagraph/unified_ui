defmodule UnifiedExamples.InlineFeedback.Screen do
  @moduledoc """
  Self-contained inline-feedback proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.InlineFeedback.Helpers
  alias UnifiedExamples.InlineFeedback.StyleProfile
  alias UnifiedExamples.InlineFeedback.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :inline_feedback_example_screen])
    tags([:example, :inline_feedback])
  end
  ...
  signals do
    ...
    interaction do
      id(:inline_feedback_example_screen_interaction)
      family(:click)
      intent(:inspect_inline_feedback)

      source_context(
        element_id: :inline_feedback_example_screen_interaction_trigger,
        scope: :screen
      )
      ...
      payload_mapping(
        ...
        example: :inline_feedback,
        outcome:
          "The review panel should explain how the inline feedback example turns an authored canonical interaction into a browser-visible feedback story.",
        source: :shared_example_trigger
      )

      summary(
        "Use the shared trigger to see how the inline feedback example explains click changes in metric or feedback meaning."
      )
    end
  end
  ...
  composition do
    root(:inline_feedback_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :inline_feedback_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :inline_feedback_example_screen_title do
        value("Inline Feedback Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :inline_feedback_example_screen_summary do
      ...
      box :inline_feedback_example_screen_panel do
        theme_ref(Theme.default_theme_id())
        ...
        inline_feedback :inline_feedback_example_primary_feedback do
          title(Helpers.inline_feedback_title())
          message(Helpers.inline_feedback_message())
          severity(Helpers.inline_feedback_severity())
          status(Helpers.inline_feedback_status())
          theme_ref(Theme.default_theme_id())
          tone(:surface)
          variant(:quiet)
        end
      ...
      button :inline_feedback_example_screen_interaction_trigger do
        label("Inspect the inline feedback feedback story")
        interaction_refs([:inline_feedback_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
      ...
      text :inline_feedback_example_screen_notes_text do
        value("Inline feedback examples foreground one canonical inline message surface inside the local shell.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.InlineFeedback.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    trigger_label: "Inspect the inline feedback feedback story",
    idle_prompt:
      "Use the shared trigger to see how the inline feedback example explains click changes in metric or feedback meaning.",
    outcome:
      "The review panel should explain how the inline feedback example turns an authored canonical interaction into a browser-visible feedback story.",
    target_surface: "inline feedback review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "Inline feedback examples foreground one canonical inline message surface inside the local shell.",
    widget: :inline_feedback,
    theme_id: @default_theme_id,
    interaction_demo: @interaction_demo
  }

  @inline_feedback_title "Validation complete"
  @inline_feedback_message "Runbook synced across regions"
  @inline_feedback_severity :success
  @inline_feedback_status :complete
  ...
  @spec inline_feedback_title() :: String.t()
  def inline_feedback_title, do: @inline_feedback_title

  @spec inline_feedback_message() :: String.t()
  def inline_feedback_message, do: @inline_feedback_message

  @spec inline_feedback_severity() :: atom()
  def inline_feedback_severity, do: @inline_feedback_severity

  @spec inline_feedback_status() :: atom()
  def inline_feedback_status, do: @inline_feedback_status
end

defmodule UnifiedExamples.InlineFeedback.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.InlineFeedback.StyleProfile do
  @moduledoc false
