defmodule UnifiedExamples.Toggle.Screen do
  @moduledoc """
  Self-contained toggle proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.Toggle.Helpers
  alias UnifiedExamples.Toggle.StyleProfile
  alias UnifiedExamples.Toggle.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :toggle_example_screen])
    tags([:example, :toggle])
  end
  ...
  signals do
    ...
    interaction do
      id(:toggle_example_screen_interaction)
      family(:change)
      intent(:inspect_toggle)

      source_context(
        element_id: :toggle_example_screen_shell,
        scope: :screen
      )
      ...
      payload_mapping(
        widget: :toggle,
        example: :toggle,
        outcome: "The review panel should explain how the toggle example turns live form input into a browser-visible change outcome.",
        source: :shared_form_shell
      )

      summary("Interact with the toggle example to see how its authored change signal updates the shared review story.")
    end
  end
  ...
  composition do
    root(:toggle_example_screen_root)
    mode(:screen)
    summary("Self-contained form-oriented example-app shell")

    form_builder :toggle_example_screen_shell do
      summary("Self-contained form-oriented example-app shell")
      theme_ref(Theme.default_theme_id())
      style_refs([:example_form_shell])
      tone(:surface)
      variant(:panel)
      interaction_refs([:toggle_example_screen_interaction])

      text :toggle_example_screen_title do
        value("Toggle Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :toggle_example_screen_summary do
      ...
      spacer :toggle_example_screen_form_gap do
        size(:sm)
      end
      ...
      text :toggle_example_screen_notes_text do
        value("Toggle examples keep the local form shell while foregrounding one boolean switch control.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.Toggle.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    trigger_label: nil,
    idle_prompt: "Interact with the toggle example to see how its authored change signal updates the shared review story.",
    outcome: "The review panel should explain how the toggle example turns live form input into a browser-visible change outcome.",
    target_surface: "toggle review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "Toggle examples keep the local form shell while foregrounding one boolean switch control.",
    widget: :toggle,
...
defmodule UnifiedExamples.Toggle.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.Toggle.StyleProfile do
  @moduledoc false
