defmodule UnifiedExamples.Column.Screen do
  @moduledoc """
  Self-contained column proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.Column.Helpers
  alias UnifiedExamples.Column.StyleProfile
  alias UnifiedExamples.Column.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :column_example_screen])
    tags([:example, :column])
  end
  ...
  signals do
    ...
    interaction do
      id(:column_example_screen_interaction)
      family(:click)
      intent(:inspect_column)

      source_context(
        element_id: :column_example_screen_interaction_trigger,
        scope: :screen
      )
      ...
      payload_mapping(
        ...
        example: :column,
        outcome:
          "The column example should make vertical sequencing and stacked actions easy to understand at a glance.",
        source: :shared_example_trigger
      )

      summary(
        "Use the shared trigger to show how the column example keeps related actions readable inside one vertical flow."
      )
    end
  end
  ...
  composition do
    root(:column_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :column_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :column_example_screen_title do
        value("Column Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :column_example_screen_summary do
      ...
      box :column_example_screen_panel do
        theme_ref(Theme.default_theme_id())
      ...
      button :column_example_screen_interaction_trigger do
        label("Review the column layout story")
        interaction_refs([:column_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
        style_refs([:example_primary_button])
        tone(:accent)
        variant(:solid)
      ...
      text :column_example_screen_notes_text do
        value("Column examples keep the local shell while foregrounding one vertical layout flow.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.Column.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    trigger_label: "Review the column layout story",
    idle_prompt:
      "Use the shared trigger to show how the column example keeps related actions readable inside one vertical flow.",
    outcome:
      "The column example should make vertical sequencing and stacked actions easy to understand at a glance.",
    target_surface: "column review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "Column examples keep the local shell while foregrounding one vertical layout flow.",
    widget: :column,
...
defmodule UnifiedExamples.Column.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.Column.StyleProfile do
  @moduledoc false
