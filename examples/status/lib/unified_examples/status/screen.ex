defmodule UnifiedExamples.Status.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    trigger_label: "Inspect the status feedback story",
    idle_prompt:
      "Use the shared trigger to see how the status example explains click changes in metric or feedback meaning.",
    outcome:
      "The review panel should explain how the status example turns an authored canonical interaction into a browser-visible feedback story.",
    target_surface: "status review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "Status examples foreground one canonical status line inside the local shell.",
    widget: :status,
    theme_id: @default_theme_id,
    interaction_demo: @interaction_demo
  }

  @status_text "Release train stable"
  ...
  @spec status_severity() :: atom()
  def status_severity, do: @status_severity

  @spec status_kind() :: atom()
  def status_kind, do: @status_kind
end

defmodule UnifiedExamples.Status.Theme do
  @moduledoc false

  alias UnifiedExamples.Status.Helpers
...
defmodule UnifiedExamples.Status.StyleProfile do
  @moduledoc false
...
defmodule UnifiedExamples.Status.Screen do
  @moduledoc """
  Self-contained status proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.Status.Helpers
  alias UnifiedExamples.Status.StyleProfile
  alias UnifiedExamples.Status.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :status_example_screen])
    tags([:example, :status])
  end
  ...
  signals do
    ...
    interaction do
      id(:status_example_screen_interaction)
      family(:click)
      intent(:inspect_status)

      source_context(
        element_id: :status_example_screen_interaction_trigger,
        scope: :screen
      )
  ...
  composition do
    root(:status_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :status_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :status_example_screen_title do
        value("Status Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :status_example_screen_summary do
      ...
      box :status_example_screen_panel do
        theme_ref(Theme.default_theme_id())
        ...
        status :status_example_primary_status do
          value(Helpers.status_text())
          severity(Helpers.status_severity())
          status(Helpers.status_kind())
          theme_ref(Theme.default_theme_id())
          tone(:surface)
          variant(:quiet)
        end
      ...
      button :status_example_screen_interaction_trigger do
        label("Inspect the status feedback story")
        interaction_refs([:status_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
      ...
      text :status_example_screen_notes_text do
        value("Status examples foreground one canonical status line inside the local shell.")
        theme_ref(Theme.default_theme_id())
