defmodule UnifiedExamples.Button.Screen do
  @moduledoc """
  Self-contained button proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.Button.Helpers
  alias UnifiedExamples.Button.StyleProfile
  alias UnifiedExamples.Button.Theme
  ...
  identity do
    ...
    description("Focused action-oriented example using the local example shell")
    authored_ref([:examples, :button_example_screen])
    tags([:example, :button])
  end
  ...
  composition do
    root(:button_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :button_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :button_example_screen_title do
        value("Button Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :button_example_screen_summary do
        value("Focused action-oriented example using the local example shell")
        theme_ref(Theme.default_theme_id())
      ...
      box :button_example_screen_panel do
        theme_ref(Theme.default_theme_id())
      ...
      text :button_example_screen_notes_text do
        value("Click the button to inspect the canonical signal compiled from the self-contained authored DSL.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.Button.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    target_surface: "button review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    title: "Button Widget Example",
    summary: "Focused action-oriented example using the local example shell",
    notes: "Click the button to inspect the canonical signal compiled from the self-contained authored DSL.",
    widget: :button,
...
defmodule UnifiedExamples.Button.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.Button.StyleProfile do
  @moduledoc false
