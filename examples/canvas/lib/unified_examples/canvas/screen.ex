defmodule UnifiedExamples.Canvas.Screen do
  @moduledoc """
  Self-contained canvas proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.Canvas.Helpers
  alias UnifiedExamples.Canvas.StyleProfile
  alias UnifiedExamples.Canvas.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :canvas_example_screen])
    tags([:example, :canvas])
  end
  ...
  signals do
    ...
    interaction do
      id(:canvas_example_screen_interaction)
      family(:focus)
      intent(:inspect_canvas)

      source_context(
        element_id: :canvas_example_screen_interaction_trigger,
        scope: :screen
      )
      ...
      payload_mapping(
        ...
        example: :canvas,
        outcome:
          "The review panel should explain how the canvas example turns an authored canonical interaction into a browser-visible display-system story.",
        source: :shared_example_trigger
      )

      summary(
        "Use the shared trigger to see how the canvas example explains focus changes in movement, focus, or rendering context."
      )
    end
  end
  ...
  composition do
    root(:canvas_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :canvas_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :canvas_example_screen_title do
        value("Canvas Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :canvas_example_screen_summary do
      ...
      box :canvas_example_screen_panel do
        theme_ref(Theme.default_theme_id())
        ...
        canvas :canvas_example_primary_canvas do
          width(72)
          height(18)
          operations(Helpers.canvas_operations())
          theme_ref(Theme.default_theme_id())
          tone(:surface)
          variant(:quiet)
        end
      ...
      button :canvas_example_screen_interaction_trigger do
        label("Inspect the canvas display story")
        interaction_refs([:canvas_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
      ...
      text :canvas_example_screen_notes_text do
        value("Canvas examples foreground one canonical drawing surface inside the local shell.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.Canvas.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    trigger_label: "Inspect the canvas display story",
    idle_prompt:
      "Use the shared trigger to see how the canvas example explains focus changes in movement, focus, or rendering context.",
    outcome:
      "The review panel should explain how the canvas example turns an authored canonical interaction into a browser-visible display-system story.",
    target_surface: "canvas review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "Canvas examples foreground one canonical drawing surface inside the local shell.",
    widget: :canvas,
  ...
  def canvas_operations, do: @canvas_operations
end

defmodule UnifiedExamples.Canvas.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.Canvas.StyleProfile do
  @moduledoc false
