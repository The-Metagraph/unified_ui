defmodule UnifiedExamples.Checkbox.Screen do
  @moduledoc """
  Self-contained checkbox proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.Checkbox.Helpers
  alias UnifiedExamples.Checkbox.StyleProfile
  alias UnifiedExamples.Checkbox.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :checkbox_example_screen])
    tags([:example, :checkbox])
  end
  ...
  signals do
    ...
    interaction do
      id(:checkbox_example_screen_interaction)
      family(:change)
      intent(:inspect_checkbox)

      source_context(
        element_id: :checkbox_example_screen_shell,
        scope: :screen
      )
      ...
      payload_mapping(
        widget: :checkbox,
        example: :checkbox,
        outcome: "The review panel should explain how the checkbox example turns live form input into a browser-visible change outcome.",
        source: :shared_form_shell
      )

      summary("Interact with the checkbox example to see how its authored change signal updates the shared review story.")
    end
  end
  ...
  composition do
    root(:checkbox_example_screen_root)
    mode(:screen)
    summary("Self-contained form-oriented example-app shell")

    form_builder :checkbox_example_screen_shell do
      summary("Self-contained form-oriented example-app shell")
      theme_ref(Theme.default_theme_id())
      style_refs([:example_form_shell])
      tone(:surface)
      variant(:panel)
      interaction_refs([:checkbox_example_screen_interaction])

      text :checkbox_example_screen_title do
        value("Checkbox Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :checkbox_example_screen_summary do
      ...
      spacer :checkbox_example_screen_form_gap do
        size(:sm)
      end
      ...
      text :checkbox_example_screen_notes_text do
        value("Checkbox examples keep the local form shell while foregrounding one boolean control.")
        theme_ref(Theme.default_theme_id())
...
defmodule UnifiedExamples.Checkbox.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    trigger_label: nil,
    idle_prompt: "Interact with the checkbox example to see how its authored change signal updates the shared review story.",
    outcome: "The review panel should explain how the checkbox example turns live form input into a browser-visible change outcome.",
    target_surface: "checkbox review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "Checkbox examples keep the local form shell while foregrounding one boolean control.",
    widget: :checkbox,
...
defmodule UnifiedExamples.Checkbox.Theme do
  @moduledoc false
...
defmodule UnifiedExamples.Checkbox.StyleProfile do
  @moduledoc false
