defmodule UnifiedExamples.Menu.Helpers do
  @moduledoc false
  ...
  @interaction_demo %{
    ...
    trigger_label: "Review the menu navigation story",
    idle_prompt:
      "Use the shared trigger to see how the menu example explains navigation changes and active-state meaning.",
    outcome:
      "The review panel should explain how the menu example turns an authored canonical interaction into a browser-visible navigation story with clear active-state meaning.",
    target_surface: "menu review panel",
    reviewer_hint:
  ...
  @metadata %{
    ...
    notes: "Menu examples foreground one canonical navigation rail inside the local shell.",
    widget: :menu,
...
defmodule UnifiedExamples.Menu.Theme do
  @moduledoc false

  alias UnifiedExamples.Menu.Helpers
...
defmodule UnifiedExamples.Menu.StyleProfile do
  @moduledoc false
...
defmodule UnifiedExamples.Menu.Screen do
  @moduledoc """
  Self-contained menu proof for the standalone example-app suite.
  """

  use UnifiedUi.Dsl

  alias UnifiedExamples.Menu.Helpers
  alias UnifiedExamples.Menu.StyleProfile
  alias UnifiedExamples.Menu.Theme
  ...
  identity do
    ...
    authored_ref([:examples, :menu_example_screen])
    tags([:example, :menu])
  end
  ...
  signals do
    ...
    interaction do
      id(:menu_example_screen_interaction)
      family(:navigation)
      intent(:inspect_menu)

      source_context(
        element_id: :menu_example_screen_interaction_trigger,
        scope: :screen
      )
  ...
  composition do
    root(:menu_example_screen_root)
    mode(:screen)
    summary("Self-contained example-app shell")

    box :menu_example_screen_shell do
      theme_ref(Theme.default_theme_id())
      style_refs([:example_shell])
      tone(:surface)
      variant(:panel)

      text :menu_example_screen_title do
        value("Menu Widget Example")
        theme_ref(Theme.default_theme_id())
      ...
      text :menu_example_screen_summary do
      ...
      box :menu_example_screen_panel do
        theme_ref(Theme.default_theme_id())
      ...
      button :menu_example_screen_interaction_trigger do
        label("Review the menu navigation story")
        interaction_refs([:menu_example_screen_interaction])
        theme_ref(Theme.default_theme_id())
      ...
      text :menu_example_screen_notes_text do
        value("Menu examples foreground one canonical navigation rail inside the local shell.")
        theme_ref(Theme.default_theme_id())
