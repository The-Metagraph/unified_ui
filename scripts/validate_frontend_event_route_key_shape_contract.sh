...
echo "Validating frontend widget event route-key payload-shape parity..."
...
if ! file_contains_literal "canonicalRouteKeyShape" "$ELM_FILE"; then
  fail "Elm harness missing canonical route-key payload-shape helper"
fi

if ! file_contains_literal "uniqueRouteKeys" "$ELM_FILE"; then
  fail "Elm harness missing route-key uniqueness helper"
fi

if ! file_contains_literal "analyzeDeclaredRouteKeys" "$BRIDGE_FILE"; then
  fail "JS bridge missing route-key payload-shape analysis helper"
fi

if ! file_contains_literal "routeKeyValueType" "$BRIDGE_FILE"; then
  fail "JS bridge missing route-key value-type helper"
fi

if ! file_contains_literal "invalid_route_keys" "$BRIDGE_FILE"; then
  fail "JS bridge missing invalid route-key diagnostics"
fi

if ! file_contains_literal "expected_value_shape" "$BRIDGE_FILE"; then
  fail "JS bridge missing expected route-key value-shape diagnostics"
fi

if ! file_contains_literal "actual_value_shape" "$BRIDGE_FILE"; then
  fail "JS bridge missing actual route-key value-shape diagnostics"
fi

if ! file_contains_literal "invalid key values for route family" "$BRIDGE_FILE"; then
  fail "JS bridge missing invalid route-key value fail-closed reason"
fi
...
if [[ "$failures" -ne 0 ]]; then
  echo
  echo "Frontend widget event route-key payload-shape validation failed."
  exit 1
fi

echo "Frontend widget event route-key payload-shape validation passed."
