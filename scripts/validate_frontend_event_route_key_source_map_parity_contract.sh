...
echo "Validating frontend widget event route-key source-map parity..."
...
if ! file_contains_literal "declaredRouteKeySourceMapParityKeys" "$ELM_FILE"; then
  fail "Elm harness missing route-key source-map parity helper"
fi
...
if ! file_contains_literal "analyzeRouteKeySourceMapKeyParityWithRouteKeys" "$BRIDGE_FILE"; then
  fail "JS bridge missing route-key source-map parity analysis helper"
fi

if ! file_contains_literal "source_map_key_parity_mismatches" "$BRIDGE_FILE"; then
  fail "JS bridge missing route-key source-map parity mismatch diagnostics"
fi

if ! file_contains_literal "route_key_sources payload key mismatch with route_keys payload for route family" "$BRIDGE_FILE"; then
  fail "JS bridge missing route-key source-map parity fail-closed reason"
fi

if ! file_contains_literal "route_key_sources_keys:" "$BRIDGE_FILE"; then
  fail "JS bridge missing source-map key diagnostics field in parity errors"
fi

if ! file_contains_literal "route_keys:" "$BRIDGE_FILE"; then
  fail "JS bridge missing route-key diagnostics field in source-map parity errors"
fi
...
if [[ "$failures" -ne 0 ]]; then
  echo
  echo "Frontend widget event route-key source-map parity validation failed."
  exit 1
fi

echo "Frontend widget event route-key source-map parity validation passed."
