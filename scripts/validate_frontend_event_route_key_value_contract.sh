...
echo "Validating frontend widget event route-key value-shape parity..."
...
if ! file_contains_literal "canonicalRouteKeyValue" "$ELM_FILE"; then
  fail "Elm harness missing canonical route-key value helper"
fi

if ! file_contains_literal "routeKeyContractStringValue" "$ELM_FILE"; then
  fail "Elm harness missing route-key contract string-value helper"
fi

if ! file_contains_literal "widgetRouteKeyStringValue" "$ELM_FILE"; then
  fail "Elm harness missing widget route-key string-value helper"
fi

if ! file_contains_literal "nonEmptyString" "$ELM_FILE"; then
  fail "Elm harness missing non-empty route-key string helper"
fi

if ! file_contains_literal "analyzeRequiredRouteKeyValues" "$BRIDGE_FILE"; then
  fail "JS bridge missing required route-key value-shape analysis helper"
fi

if ! file_contains_literal "expected_route_key_value_shape" "$BRIDGE_FILE"; then
  fail "JS bridge missing expected route-key value-shape diagnostics"
fi

if ! file_contains_literal "invalid_route_key_values" "$BRIDGE_FILE"; then
  fail "JS bridge missing invalid route-key value diagnostics"
fi

if ! file_contains_literal "route-key payload values must be non-empty strings for route family" "$BRIDGE_FILE"; then
  fail "JS bridge missing route-key value-shape fail-closed reason"
fi
...
if [[ "$failures" -ne 0 ]]; then
  echo
  echo "Frontend widget event route-key value-shape validation failed."
  exit 1
fi

echo "Frontend widget event route-key value-shape validation passed."
