...
echo "Validating frontend widget event route-keys continuity parity..."
...
if ! file_contains_literal "declaredRouteKeys" "$ELM_FILE"; then
  fail "Elm harness missing declared route-keys helper"
fi

if ! file_contains_literal "isRouteKeyPopulated" "$ELM_FILE"; then
  fail "Elm harness missing route-key population helper"
fi

if ! file_contains_literal "route_keys" "$BRIDGE_FILE"; then
  fail "JS bridge missing route_keys payload continuity field handling"
fi

if ! file_contains_literal "normalizedStringList" "$BRIDGE_FILE"; then
  fail "JS bridge missing route_keys normalization helper"
fi
...
if ! file_contains_literal "expected_route_keys" "$BRIDGE_FILE"; then
  fail "JS bridge missing expected route_keys diagnostics"
fi

if ! file_contains_literal "actual_route_keys" "$BRIDGE_FILE"; then
  fail "JS bridge missing actual route_keys diagnostics"
fi

if [[ "$failures" -ne 0 ]]; then
  echo
  echo "Frontend widget event route-keys contract validation failed."
  exit 1
fi

echo "Frontend widget event route-keys contract validation passed."
