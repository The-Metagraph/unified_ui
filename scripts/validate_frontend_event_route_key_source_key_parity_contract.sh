...
if ! file_contains_literal "declaredRouteKeySourceParityKeys" "$ELM_FILE"; then
  fail "Elm harness missing declared route-key source-key parity helper"
fi
...
if ! file_contains_literal "analyzeRouteKeySourceKeyParityWithRouteKeys" "$BRIDGE_FILE"; then
  fail "JS bridge missing route-key source-key parity analysis helper"
fi

if ! file_contains_literal "source_key_parity_mismatches" "$BRIDGE_FILE"; then
  fail "JS bridge missing route-key source-key parity mismatch diagnostics"
fi

if ! file_contains_literal "route_key_source_keys payload mismatch with route_keys payload for route family" "$BRIDGE_FILE"; then
  fail "JS bridge missing route-key source-key parity fail-closed reason"
fi

if ! file_contains_literal "route_keys:" "$BRIDGE_FILE"; then
  fail "JS bridge missing route-key diagnostics field in source-key parity errors"
fi

if ! file_contains_literal "route_key_source_keys:" "$BRIDGE_FILE"; then
  fail "JS bridge missing source-key diagnostics field in source-key parity errors"
fi
...
if [[ "$failures" -ne 0 ]]; then
  echo
  echo "Frontend widget event route-key source-key parity validation failed."
  exit 1
fi

echo "Frontend widget event route-key source-key parity validation passed."
