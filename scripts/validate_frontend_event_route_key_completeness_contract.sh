...
echo "Validating frontend widget event route-key completeness parity..."
...
if ! file_contains_literal "routeFamilyRequirementValue" "$ELM_FILE"; then
  fail "Elm harness missing route-key completeness helper"
fi

if ! file_contains_literal "missingRouteKeys" "$BRIDGE_FILE"; then
  fail "JS bridge missing missing-route-keys completeness helper"
fi

if ! file_contains_literal "missing_route_keys" "$BRIDGE_FILE"; then
  fail "JS bridge missing missing_route_keys diagnostics"
fi

if ! file_contains_literal "missing required route keys for route family" "$BRIDGE_FILE"; then
  fail "JS bridge missing typed missing-required-route-keys failure reason"
fi

if ! file_contains_literal "expected_route_keys: requiredRouteKeys" "$BRIDGE_FILE"; then
  fail "JS bridge missing canonical required route-key expectation diagnostics"
fi

if ! file_contains_literal "actual_route_keys: presentRouteKeys" "$BRIDGE_FILE"; then
  fail "JS bridge missing canonical present route-key diagnostics"
fi
...
if [[ "$failures" -ne 0 ]]; then
  echo
  echo "Frontend widget event route-key completeness validation failed."
  exit 1
fi

echo "Frontend widget event route-key completeness validation passed."
