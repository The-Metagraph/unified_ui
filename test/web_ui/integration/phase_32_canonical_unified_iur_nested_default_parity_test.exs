defmodule WebUi.Integration.Phase32CanonicalUnifiedIurNestedDefaultParityTest do
  ...
  test "SCN-037 equivalent canonical nested inputs with default-profile differences produce identical snapshots" do
    ...
    table_node = Enum.find(interpreted_struct.root.children, &(&1.id == "orders_table"))

    assert button_node.props.style == %{fg: :blue}
    assert table_node.props.columns == [%{key: :id, header: "ID"}]
  end

  test "SCN-037 malformed nested payloads fail closed with typed validation errors" do
    malformed = %{
      type: :vbox,
      children: [
    ...
    assert {:error, %TypedError{} = error} =
             Interpreter.interpret(malformed, correlation_id: "corr-scn-037")

    assert error.error_code == "iur.interpreter.signal_mapping_failed"
    assert error.correlation_id == "corr-scn-037"
  end

  test "SCN-037 repeated equivalent interpretation flows produce identical snapshots" do
    flow_snapshot = fn ->
